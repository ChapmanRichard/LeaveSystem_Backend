using Api.Common.AntiXSS.Model;
...
{
    public sealed class AntiXSSDomScanner
    {
        private readonly List<string> _errorMessages = new List<string>();

        private readonly Policy _policy;

        public AntiXSSDomScanner(Policy policy) => _policy = policy;

        public AntiyXSSResult Scan(string html)
        {
            if (html == null)
            {
                return new AntiyXSSResult(DateTime.Now, html, _errorMessages);
            }
            ...
            return new AntiyXSSResult(start, finalCleanHtml, _errorMessages);
        }
