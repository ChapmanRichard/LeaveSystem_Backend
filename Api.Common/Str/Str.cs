...
namespace Api.Common
{
    public static class Str
    {
        ...
        public static bool IsLettersOrNum(string str)
        {
        ...
        /// <param name="str">要判断的字符串</param>
        /// <returns>判断结果</returns>
        public static bool IsLettersOrNumOrHeng(string str)
        {
            return Regex.IsMatch(str, @"^[0-9a-zA-Z\-| ]+$");
        }
        ...
        /// <param name="str">要判断的字符串</param>
        /// <returns>判断结果</returns>
        public static bool IsInts(string str)
        {
        ...
        /// <returns></returns>
        public static string RemoveNotAssic(string srcStr)
        {
            return Regex.Replace(srcStr, @"[^\u0000-\u007F]+", string.Empty);
        }
        /// <summary>
        /// 逆置字符串(短)
        /// </summary>
        ...
        #region 处理逗号分隔的字符串可为SQL IN 条件的直接使用的参数
        /// <summary>
        /// e.g "1,2"=>"'1','2'"
        /// </summary>
        /// <param name="strCode"></param>
        /// <returns></returns>
        public static string CharacterProcessing(string strCode)
        {
            var newids = string.Empty;
            if (strCode.Split(',').Length > 1)
            {
                string ids = strCode.Replace(",", "','");
                newids += "'" + ids + "'";
            }
            else
            {
        ...
        /// </summary>
        /// <param name="srcStr">源字符串</param>
        /// <param name="length">目标字符串的位数</param>
        /// <param name="mode">模式：L为左边补位，R为右边补位</param>
        /// <param name="genre">类型：补位的字符</param>
        /// <returns></returns>
        public static string GetFixedLength(string srcStr, int length, string mode = "L", char genre = '0')
        {
            if (mode != "L")
            {
                return srcStr.PadRight(length, genre);
            }
            else
            {
                return srcStr.PadLeft(length, genre);
        ...
        /// </summary>
        /// <param name="srcStr">源字符串</param>
        /// <param name="mode">模式：0为去除去掉字符串前后的空格；1为去掉字符串中间及前后的空格；2为兼容任何空白符，匹配符\s(匹配任何空白字符，包括空格，制表符，换页符等，与[\f\n\t\r\v]等效)；3为去除去掉字符串前面的空格；4为去除去掉字符串后面的空格。</param>
        /// <returns></returns>
        public static string GetTrim(string srcStr, int mode = 0)
        {
            switch (mode)
            {
                case 0:
                ...
                case 2:
                    return Regex.Replace(srcStr, @"\s", "");
                case 3:
                    return srcStr.TrimStart();
                case 4:
                    return srcStr.TrimEnd();
                default:
                    return srcStr.Trim();
            }
        }
        /// <summary>
        /// 判断字符串里是否包含括号对（兼容中文括号和英文括号）（以第一个左括号和最后一个右括号确定为括号对）
        /// </summary>
        /// <param name="str"></param>
        /// <returns></returns>
        public static bool is_match(string str, out int cntStar, out int cntEnd)
        {
            cntStar = -1;
            cntEnd = -1;
        ...
        /// 移除多于空格和回车
        /// </summary>
        /// <param name="stringBuilder">源字符串</param>
        /// <returns></returns>
        public static StringBuilder RemoveEmpty(StringBuilder stringBuilder)
        {
            if (stringBuilder == null || stringBuilder.Length <= 0) return stringBuilder;
            return stringBuilder.Replace("\r", " ").Replace("\n", " ").Replace("\t", " ").Replace("  ", " ");
        }
        /// <summary>
        /// 获取数组某个位置的值(默认第二个值)
        /// </summary>
        /// <param name="str"></param>
        /// <returns></returns>
        public static string GetArrSecond(string str, int i = 1)
        {
            if (!str.IsEmpty())
            {
                var arr = str.Split(',');
