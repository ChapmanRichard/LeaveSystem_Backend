...
namespace Api.Common.Mappers
{
    ...
    public sealed class DTOrder
    {
        ...
        /// <summary>
        /// Column to which ordering should be applied. This is an column's Name reference to the 
        /// columns array of information that is also submitted to the server
        /// </summary>
    ...
    public class DTModelBinder : IModelBinder
    {
        ...
        public Task BindModelAsync(ModelBindingContext bindingContext)
        {
            ...
            var draw = Convert.ToInt32(valueProvider.GetValue("draw").FirstOrDefault());
            var start = Convert.ToInt32(valueProvider.GetValue("start").FirstOrDefault());
            var length = Convert.ToInt32(valueProvider.GetValue("length").FirstOrDefault());
            // Search
            var search = new DTSearch
            {
                Value = Convert.ToString(valueProvider.GetValue("search[value]").FirstOrDefault()),
                Regex = Convert.ToBoolean(valueProvider.GetValue("search[regex]").FirstOrDefault())
            };
            // Order
            var o = 0;
            var order = new List<DTOrder>();
            while (valueProvider.GetValue("order[" + o + "][column]") != ValueProviderResult.None)
            {
                order.Add(new DTOrder
                {
                    Column = Convert.ToInt32(valueProvider.GetValue("order[" + o + "][column]").FirstOrDefault()),
                    Dir = valueProvider.GetValue("order[" + o + "][dir]").FirstOrDefault().ToString()
                });
                o++;
            }
            // Columns
            ...
            while (valueProvider.GetValue("columns[" + c + "][name]") != ValueProviderResult.None)
            {
                columns.Add(new DTColumn
                {
                    Index = c,
                    Data = valueProvider.GetValue("columns[" + c + "][data]").FirstOrDefault().ToString(),
                    Name = valueProvider.GetValue("columns[" + c + "][name]").FirstOrDefault().ToString(),
                    Orderable = Convert.ToBoolean(valueProvider.GetValue("columns[" + c + "][orderable]").FirstOrDefault()),
                    Searchable = Convert.ToBoolean(valueProvider.GetValue("columns[" + c + "][searchable]").FirstOrDefault()),
                    Search = new DTSearch
                    {
                        Value = valueProvider.GetValue("columns[" + c + "][search][value]").FirstOrDefault().ToString(),
                        Regex = Convert.ToBoolean(valueProvider.GetValue("columns[" + c + "][search][regex]").FirstOrDefault())
                    }
            ...
            var model = new DTParameterModel
            {
                Draw = draw,
                Start = start,
                Length = length,
